...
(function() {
	...
	const updateMode = () => {
		...
		if (sidebarTypeSession !== null && sidebarTypeSession !== undefined) {
			...
			Array.from(sidebarTypeSetting, (type) => {
				document
					.querySelectorAll(`[data-setting="sidebar"][data-name="sidebar-type"][data-value="${type}"]`)
					.forEach((el) => {
						el.classList.add('active');
					});
	...
	Array.from(dirMode, (mode) => {
		mode.addEventListener('click', (e) => {
			...
			if (!mode.classList.contains('active')) {
				Array.from(document.querySelectorAll(`[data-value="${mode.getAttribute('data-value')}"]`), (el) =>
					el.classList.add('active')
				);
			...
			if (sidebarType.classList.contains('active')) {
				...
				if (sidebarType.getAttribute('data-extra-value') !== null) {
					document
						...
						.classList.remove(sidebarType.getAttribute('data-extra-value'));
					document
						.querySelector(`[data-value="${sidebarType.getAttribute('data-extra-value')}"]`)
						.classList.remove('active');

					const sidebarExtraTypeIndexOf = sidebarTypeSetting.findIndex(
						(type) => type === sidebarType.getAttribute('data-extra-value')
					);
					sidebarTypeSetting.splice(sidebarExtraTypeIndexOf, 1);
				}
			} else {
				Array.from(
					document.querySelectorAll(`[data-value="${sidebarType.getAttribute('data-value')}"]`),
				...
				if (sidebarType.getAttribute('data-extra-value') !== null) {
					...
					Array.from(
						document.querySelectorAll(`[data-value="${sidebarType.getAttribute('data-extra-value')}"]`),
						(el) => el.classList.add('active')
					);
	...
	Array.from(allActiveType, (activeStyle) => {
		activeStyle.addEventListener('click', (e) => {
			if (!activeStyle.classList.contains('active')) {
				...
				Array.from(
					document.querySelectorAll(`[data-value="${activeStyle.getAttribute('data-value')}"]`),
					(el) => el.classList.add('active')
				);
	...
	Array.from(allNavbarType, (navbarType) => {
		navbarType.addEventListener('click', (e) => {
			...
			Array.from(document.querySelectorAll(`[data-value="${navbarType.getAttribute('data-value')}"]`), (el) =>
				el.classList.add('active')
			);
			document
