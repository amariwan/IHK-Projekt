...
function instanceWatch(source, value, options) {
	...
	const getter = isString(source)
		? source.includes('.') ? createPathGetter(publicThis, source) : () => publicThis[source]
				...
				if (domType === 1 /* DOMNodeTypes.ELEMENT */ || domType === 3 /* DOMNodeTypes.TEXT */) {
					...
					for (let i = 0; i < vnode.staticCount; i++) {
						if (needToAdoptContent)
							vnode.children +=
								nextNode.nodeType === 1 /* DOMNodeTypes.ELEMENT */ ? nextNode.outerHTML : nextNode.data;
	...
	const handleMismatch = (node, vnode, parentComponent, parentSuspense, slotScopeIds, isFragment) => {
		...
		warn$1(
			...
			node.nodeType === 3 /* DOMNodeTypes.TEXT */
				? `(text)`
				: isComment(node) && node.data === '[' ? `(start of fragment)` : ``
		);
		vnode.el = null;
		if (isFragment) {
...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const move = (vnode, container, anchor, moveType, parentSuspense = null) => {
		...
		const needTransition =
			moveType !== 2 /* MoveType.REORDER */ && shapeFlag & 1 /* ShapeFlags.ELEMENT */ && transition;
		if (needTransition) {
...
function cloneVNode(vnode, extraProps, mergeRef = false) {
	...
	const cloned = {
		...
		children:
			patchFlag === -1 /* PatchFlags.HOISTED */ && isArray(children) ? children.map(deepCloneVNode) : children,
		target: vnode.target,
...
function cloneIfMounted(child) {
	return (child.el === null && child.patchFlag !== -1) /* PatchFlags.HOISTED */ || child.memo
		? child
		: cloneVNode(child);
}
...
const patchProp = (
	...
	if (key === 'class') {
		...
		key[0] === '.'
			...
			: key[0] === '^' ? ((key = key.slice(1)), false) : shouldSetAsProp(el, key, nextValue, isSVG)
	) {
...
function hasProp(prop, props) {
	...
	if (prop.key.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */) {
		...
		result = props.properties.some(
			(p) => p.key.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */ && p.key.content === propKeyName
		);
	}
	return result;
}
				...
				if (!context.inPre) {
					if (!/[^\t\r\n\f ]/.test(node.content)) {
						...
						if (
							...
							!next ||
							(shouldCondense &&
								((prev.type === 3 /* NodeTypes.COMMENT */ && next.type === 3) /* NodeTypes.COMMENT */ ||
...
function genAssets(assets, type, { helper, push, newline, isTS }) {
	...
	for (let i = 0; i < assets.length; i++) {
		...
		push(
			`const ${toValidAssetId(id, type)} = ${resolver}(${JSON.stringify(id)}${maybeSelfReference
				? `, true`
				: ``})${isTS ? `!` : ``}`
		);
...
function genObjectExpression(node, context) {
	...
	const multilines =
		properties.length > 1 || properties.some((p) => p.value.type !== 4 /* NodeTypes.SIMPLE_EXPRESSION */);
	push(multilines ? `{` : `{ `);
	multilines && indent();
	for (let i = 0; i < properties.length; i++) {
		const { key, value } = properties[i];
					...
					if (key) {
						sibling.branches.forEach(({ userKey }) => {
							if (isSameKey(userKey, key)) {
								context.onError(
									createCompilerError(29 /* ErrorCodes.X_V_IF_SAME_KEY */, branch.userKey.loc)
								);
							}
						});
					}
...
function createChildrenCodegenNode(branch, keyIndex, context) {
	...
	const keyProperty = createObjectProperty(
		`key`,
		createSimpleExpression(`${keyIndex}`, false, locStub, 2 /* ConstantTypes.CAN_HOIST */)
	);
...
const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
	...
	return processFor(node, dir, context, (forNode) => {
		...
		const fragmentFlag = isStableFragment
			? 64 /* PatchFlags.STABLE_FRAGMENT */
			: keyProp ? 128 /* PatchFlags.KEYED_FRAGMENT */ : 256 /* PatchFlags.UNKEYED_FRAGMENT */;
			...
			if (isTemplate) {
				node.children.some((c) => {
					if (c.type === 1 /* NodeTypes.ELEMENT */) {
						...
						if (key) {
							context.onError(
								createCompilerError(33 /* ErrorCodes.X_V_FOR_TEMPLATE_KEY_PLACEMENT */, key.loc)
							);
							return true;
						}
					}
...
const trackSlotScopes = (node, context) => {
	if (
		node.type === 1 /* NodeTypes.ELEMENT */ &&
		(node.tagType === 1 /* ElementTypes.COMPONENT */ || node.tagType === 3) /* ElementTypes.TEMPLATE */
	) {
...
function buildSlots(node, context, buildSlotFn = buildClientSlotFn) {
	...
	const slotFlag = hasDynamicSlots
		? 2 /* SlotFlags.DYNAMIC */
		: hasForwardedSlots(node.children) ? 3 /* SlotFlags.FORWARDED */ : 1 /* SlotFlags.STABLE */;
		...
		if (node.children.length > 0) {
			if (vnodeTag === KEEP_ALIVE) {
				...
				if (node.children.length > 1) {
					context.onError(
						createCompilerError(46 /* ErrorCodes.X_KEEP_ALIVE_INVALID_CHILDREN */, {
			...
			if (shouldBuildAsSlots) {
				...
				const hasDynamicTextChild =
					type === 5 /* NodeTypes.INTERPOLATION */ || type === 8 /* NodeTypes.COMPOUND_EXPRESSION */;
...
function buildDirectiveArgs(dir, context) {
	...
	if (Object.keys(dir.modifiers).length) {
		...
		dirArgs.push(
			createObjectExpression(dir.modifiers.map((modifier) => createObjectProperty(modifier, trueExpression)), loc)
		);
	}
...
function processSlotOutlet(node, context) {
	...
	if (nonNameProps.length > 0) {
		...
		if (directives.length) {
			context.onError(
				createCompilerError(36 /* ErrorCodes.X_V_SLOT_UNEXPECTED_DIRECTIVE_ON_SLOT_OUTLET */, directives[0].loc)
			);
		}
	}
	return {
...
const transformOn = (dir, node, context, augmentor) => {
	...
	if (arg.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */) {
		if (arg.isStatic) {
			...
			const eventString =
				node.tagType !== 0 /* ElementTypes.ELEMENT */ || rawName.startsWith('vnode') || !/[A-Z]/.test(rawName)
				...
				(children.length === 1 &&
					(node.type === 0 /* NodeTypes.ROOT */ ||
						...
						node.tagType === 0 /* ElementTypes.ELEMENT */ &&
							...
							!node.props.find(
								(p) => p.type === 7 /* NodeTypes.DIRECTIVE */ && !context.directiveTransforms[p.name]
							) &&
...
const transformModel = (dir, node, context) => {
	...
	const eventName = arg
		? isStaticExp(arg) ? `onUpdate:${arg.content}` : createCompoundExpression([ '"onUpdate:" + ', arg ])
		: `onUpdate:modelValue`;
	let assignmentExp;
	...
	if (dir.modifiers.length && node.tagType === 1 /* ElementTypes.COMPONENT */) {
		...
		const modifiersKey = arg
			? isStaticExp(arg) ? `${arg.content}Modifiers` : createCompoundExpression([ arg, ' + "Modifiers"' ])
			: `modelModifiers`;
		props.push(
			createObjectProperty(
				modifiersKey,
				createSimpleExpression(`{ ${modifiers} }`, false, dir.loc, 2 /* ConstantTypes.CAN_HOIST */)
			)
		);
	}
...
const transformModel$1 = (dir, node, context) => {
				...
				if (type.type === 7 /* NodeTypes.DIRECTIVE */) {
					...
					switch (type.value.content) {
						...
						case 'file':
							isInvalidType = true;
							context.onError(
								createDOMCompilerError(57 /* DOMErrorCodes.X_V_MODEL_ON_FILE_INPUT_ELEMENT */, dir.loc)
							);
							break;
						default:
							// text type
	...
	baseResult.props = baseResult.props.filter(
		(p) => !(p.key.type === 4 /* NodeTypes.SIMPLE_EXPRESSION */ && p.key.content === 'modelValue')
	);
	return baseResult;
};
		...
		if (component === TRANSITION$1) {
			return () => {
				...
				if (hasMultipleChildren(node)) {
					context.onError(
						createDOMCompilerError(60 /* DOMErrorCodes.X_TRANSITION_INVALID_CHILDREN */, {
							start: node.children[0].loc.start,
							end: node.children[node.children.length - 1].loc.end,
							source: ''
						})
