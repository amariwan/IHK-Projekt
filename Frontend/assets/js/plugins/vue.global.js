				...
				const doSet = () => {
					if (rawRef.f) {
						const existing = _isString
							? hasOwn(setupState, ref) ? setupState[ref] : refs[ref]
							: ref.value;
						if (isUnmount) {
   ...
   */
	function validateBrowserExpression(node, context, asParams = false, asRawStatements = false) {
		...
		try {
			...
			context.onError(
				createCompilerError(45 /* ErrorCodes.X_INVALID_EXPRESSION */, node.loc, undefined, message)
			);
		}
	}
					...
					(children.length === 1 &&
						(node.type === 0 /* NodeTypes.ROOT */ ||
							...
							node.tagType === 0 /* ElementTypes.ELEMENT */ &&
								...
								!node.props.find(
									(p) =>
										p.type === 7 /* NodeTypes.DIRECTIVE */ && !context.directiveTransforms[p.name]
								) &&
				...
				for (let i = 0; i < children.length; i++) {
					...
					if (isText(child) || child.type === 8 /* NodeTypes.COMPOUND_EXPRESSION */) {
						...
						if (!context.ssr && getConstantType(child, context) === 0 /* ConstantTypes.NOT_CONSTANT */) {
							callArgs.push(
								1 /* PatchFlags.TEXT */ + ` /* ${PatchFlagNames[1 /* PatchFlags.TEXT */]} */`
							);
						}
						children[i] = {
