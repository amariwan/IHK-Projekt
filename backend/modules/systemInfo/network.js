...
module.exports = class NetworkMetrics {
	constructor(io, conf) {
		...
		this.interfaceNames = Object.keys(os.networkInterfaces()).filter((networkName) => networkName !== 'lo');
				...
				return ifConfigFetch((err) => {
					...
					if (err)
						return console.error(
							`No network tool found to parse network data, stopping network monitoring`
						);
	...
	fetch(fetchImpl) {
		...
		fetchImpl((err, interfaces) => {
			...
			const values = interfaces.map((networkInterface) => {
				...
				const oldValues = this.cache.find(
					(oldInterfaceValue) => oldInterfaceValue.name === networkInterface.name
				);
				...
				return {
					name: networkInterface.name,
					inbound: ((networkInterface.inbound - oldValues.inbound) / this.refreshInterval / 1000000).toFixed(
						2
					),
					outbound: ((networkInterface.outbound - oldValues.outbound) /
